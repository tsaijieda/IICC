...
{\LARGE\bfseries 戰術復刻　戰術語言一覽\par}
\vspace{0.4em}
{\large 傳球點模式：每次接球記錄「點」（接球者＋位置）與「邊」（傳球方式）\par}
\vspace{1em}

\section*{一、基本規則}

\begin{itemize}
  \item 未符合下列任何特殊傳球時，一律標為 \term{傳球}。
  \item 戰術描述寫法：依序寫明「傳球人將球【邊】給接球人，接球人在【位置】」；終點可標【射正】或【得分】。
  \item 轉譯時若找不到相符名詞，該段邊記為「無」。
  \item 場地 20 個戰術區編號與中文名稱見 \texttt{zone\_map.pdf}。
\end{itemize}
...
\vspace{0.5em}
\noindent
部分題目允許在弧頂前緣或邊路起腳標為射正，依該題戰術文字為準。

\section*{四、戰術區 20 格名稱}

完整示意見 \texttt{zone\_map.pdf}。編號與名稱對照：
...
\begin{quote}
邊後衛將球傳球給前腰，前腰在右肋接球。前腰將球傳球給邊後衛，邊後衛在右路底線接球。邊後衛傳中給前鋒，前鋒在禁區中央得分。
\end{quote}

\vspace{1em}
\hrule
\vspace{0.4em}
{\small 規則來源：\texttt{1st\_half\_rules.md}；自動轉譯邏輯見 \texttt{tactic\_translate/}。}
