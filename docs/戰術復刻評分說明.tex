% 編譯：cd docs && xelatex 戰術復刻評分說明.tex
...
\vspace{0.6em}
\noindent
本文件說明上半場 \term{戰術復刻} 之評分方式。正式規則以 \texttt{1st\_half\_rules.md} 為準；本文件為參賽隊伍之評分摘要。

\section*{1.\ 總分結構}

本關共 \term{6 題戰術}，每題限時 \term{3 分鐘}。每題分為三項子分：
...
\vspace{0.4em}
\noindent
\textbf{合計：450 分}（75 分／題 $\times$ 6 題）。
...
\vspace{0.4em}
\noindent
球心未落於格子內者，該拍 \term{不予計分}。

\section*{4.\ 場上防守球員}

\begin{itemize}
  \item 場上配置約 \term{10 名} 靜止防守球員（圓柱體），開題前依題目示意圖擺設。
  \item 防守球員 \term{全程不移動}；其連線構成防線。
  \item 傳球路徑與圓柱接觸者，該次傳球判定失敗，須重新執行。
\end{itemize}
