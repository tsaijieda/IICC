...
## 關卡流程及場地圖
![關卡流程](../relativiskick_flowchart_v2.png)

![場地圖](../relativiskick_layout.png)
足球場中的藍線是上半場專用，白線及邊線是下半場專用。

## 闖關規則
1. 本關卡共80分鐘，前五分鐘為讀題時間，後續時間分配如關卡流程所示。
2. 讀題時間結束後可打王牌或魔法牌
3. 上半場開始時的建議分工：戰術復刻約2-3人，戰術研判約2-3人。
4. 破壞場佈則整關0分。
...
3. 依據文本題得分，會得到等量的足球點數，可在解防守題時購買回復牌與迴轉牌（每張售價為100個足球點數）

其中「相對同時性與因果律」題組最重要，其最終目標為解出解防守題的「相對論規則」。若在上半場結束前有成功解出，則可進行「戰術升級」，將會降低下半場解防守題的難度。

## 中場休息
上半場結束後，會進入5分鐘的中場休息。此時關主會計算上半場的得分（包含獲得足球點數的數量以及戰術升級是否成功）。建議可利用這段時間研究下半場解防守題的規則。完成後會公布。

## 解防守題
為回合制遊戲，一共有五題防守題，一題180分，共900分。需依照場上形勢選擇戰術，並規劃跑位來解決問題。足球場上有數名防守球員，各具不同的性質，包括瘋狗逼搶員、區域大閘、影子盯人者、路線攔截者及守門員。各題的防守球員配置不一（詳見防守題評分方式），可自行決定解題順序。在開始解每一題前需告知關主以下資訊：
1. 本題要上場的四名隊員（未上場隊員為替補球員）
2. 是否購買回復牌及迴轉牌
...
得到黃牌後，場上狀態會復原至該階段開始前。所有防守題中，每一題的黃牌數會分開累計。
...
### 卡牌功能
- 回復牌：回合中任何階段皆可使用，場上狀態會回到該回合開始前
- 迴轉牌：得到該題的第二張黃牌時可使用，使用後場上狀態會復原至該階段開始前
...
![成就分數](accomplishment_table.png)

破壞場佈時以零分計算，打魔法牌分數加倍，打王牌以400分計算。
